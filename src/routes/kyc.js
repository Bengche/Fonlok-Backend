...
router.post("/submit", authMiddleware, (req, res) => {
  kycUpload(req, res, async (err) => {
    ...
    if (!validDocTypes.includes(docType)) {
      return res.status(400).json({ message: "Invalid document type. Choose national_id, drivers_license, or passport." });
    }
    if (!fullName || fullName.length < 3) {
      return res.status(400).json({ message: "Full name is required (minimum 3 characters)." });
    }
    if (!validateDob(dob)) {
      return res.status(400).json({ message: "A valid date of birth is required. You must be at least 16 years old." });
    }
    if (!nationality) return res.status(400).json({ message: "Nationality is required." });
    if (!phone)       return res.status(400).json({ message: "Phone number is required." });
    if (!address)     return res.status(400).json({ message: "Residential address is required." });
    if (!city)        return res.status(400).json({ message: "City is required." });
    if (!docNumber)   return res.status(400).json({ message: "Document number is required." });
    ...
    if (docType !== "passport" && !files.document_back?.[0]) {
      return res.status(400).json({ message: "Please upload the back of your identity document." });
    }
    if (!files.selfie?.[0]) {
      return res.status(400).json({ message: "Please upload a selfie photograph." });
    }
    ...
    try {
      const frontResult = await uploadToCloudinary(files.document_front[0].buffer, {
        folder,
        resource_type: "image",
      ...
      if (docType !== "passport" && files.document_back?.[0]) {
        const backResult = await uploadToCloudinary(files.document_back[0].buffer, {
          folder,
          resource_type: "image",
      ...
      logger.error("KYC Cloudinary upload failed", { error: uploadErr.message });
    ...
    try {
      if (existingRow.rows.length && existingRow.rows[0].status === "rejected") {
        const old = await db.query("SELECT * FROM kyc_verifications WHERE id = $1", [existingRow.rows[0].id]);
        const o = old.rows[0];
        ...
        await db.query(
          `UPDATE kyc_verifications
           ...
           WHERE id=$16`,
          [fullName, dob, nationality, phone, address, city, country, docType, docNumber,
           frontUrl, frontPid, backUrl || null, backPid || null,
           selfieUrl, selfiePid, existingRow.rows[0].id],
        );
      } else {
        // Fresh submission
        await db.query(
          ...
          [userId, fullName, dob, nationality, phone, address, city, country,
           docType, docNumber,
           frontUrl, frontPid, backUrl || null, backPid || null,
           selfieUrl, selfiePid],
        );
    ...
    try {
      const uRes = await db.query("SELECT email, name, username FROM users WHERE id=$1", [userId]);
    ...
    if (userEmail && process.env.SENDGRID_API_KEY?.startsWith("SG.")) {
      const docTypeLabel = { national_id: "National ID Card", drivers_license: "Driver's Licence", passport: "International Passport" }[docType] || docType;
      const body = `
        ...
        ${emailTable([
          ...
          ["Date Submitted",  new Date().toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" })],
          ["Current Status",  '<span style="background:#fef3c7;color:#92400e;padding:3px 10px;border-radius:999px;font-weight:700;font-size:12px;">Under Review</span>', ""],
        ])}
    ...
    return res.status(200).json({ message: "Verification submitted successfully. We will review your documents within 1–3 business days.", status: "pending" });
  });
});
